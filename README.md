...
- [PickySteve](https://github.com/KernelLord/pickysteve) - Skill router and context picker for coding agents: hybrid BM25+embedding retrieval (RRF-fused), cross-encoder rerank, and an LLM judge route each prompt to the right skill, then assemble a minimal context bundle. Fail-closed prompt-injection gate scans both the request and every retrieved document. Runs fully local on Ollama; MCP server + OpenAI-compatible proxy install into 18 coding agents (Claude Code, Cursor, Codex, etc.). MIT licensed.
...
**Mastering AI Agent Evaluation** (Free eBook)  
*Future AGI* [[link](https://futureagi.com/mastering-ai-agent-evaluation)]
